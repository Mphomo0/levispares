...
export default function EditBrandPage({
  ...
  const deleteImageByFileId = useAction(api.imageActions.deleteImageByFileId)
  ...
  useEffect(() => {
    if (brand && !initializedRef.current) {
      initializedRef.current = true
      setLogo(brand.logo)
      setLogoFileId(brand.imageKitFileId)
    }
  }, [brand])
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Delete old image if logo is being replaced
      if (logo !== brand.logo && brand.imageKitFileId) {
        await deleteImageByFileId({ fileId: brand.imageKitFileId })
      }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Modify brand details and settings.
          </p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload or update the brand logo.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <LogoUpload
                value={logo}
                onChange={(url, fileId) => {
                  setLogo(url || undefined)
                  setLogoFileId(fileId || undefined)
                }}
                folder="/brands"
                label=""
              />
